...
def _draw_graph(axis, graph):
    if not (graph.x_error is None and graph.y_error is None):
        ...
        line = axis.plot(graph.x_values, graph.y_values, graph.plot_type, ms=13, color=graph.color)[0]
    if graph.curve_fits is not None:
        for fit in graph.curve_fits:
            ...
            fit.values = optimal
            fit.dvalues = stddev
            regression = axis.plot(graph.x_values, fit.func(graph.x_values, *optimal))[0]
